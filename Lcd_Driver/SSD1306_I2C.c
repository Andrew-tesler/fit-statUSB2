...
#include "util\font12x16.h"
#include "driverlib.h"
//#include "TI_USCI_I2C_master.h"

unsigned char *ssd1306_buf1;
...
{
  ssd1306_buf2[0] = SSD1306_COMMAND_MODE;
  ssd1306_buf2[1] = command;
  
  USCI_B_I2C_masterSendMultiByteStart(USCI_B0_BASE,ssd1306_buf2[0]);

  USCI_B_I2C_masterSendMultiByteNext(USCI_B0_BASE,ssd1306_buf2[1]);

  USCI_B_I2C_masterSendMultiByteStop(USCI_B0_BASE);

//  TI_USCI_I2C_transmitinit(SSD1306_ADDRESS);  // init transmitting with USCI
//  while (TI_USCI_I2C_notready());             // wait for bus to be free
//  TI_USCI_I2C_transmit(2, ssd1306_buf2);      // command mode
//  while (TI_USCI_I2C_notready());             // wait for bus to be free
}
...
//           USCI_B_I2C_masterSendMultiByteStart(USCI_B0_BASE,control);
           ...
           //
           count--;                // decrement byte counter
       }
    USCI_B_I2C_masterSendMultiByteStop(USCI_B0_BASE);
//  TI_USCI_I2C_transmitinit(SSD1306_ADDRESS);  // init transmitting with USCI
//  while (TI_USCI_I2C_notready());             // wait for bus to be free
//  TI_USCI_I2C_transmit(count, data);          // data mode
//  while (TI_USCI_I2C_notready());             // wait for bus to be free
}
...
void ssd1306_FillDisplay(unsigned char data)
{
  ...
  ssd1306_buf1 = malloc(SSD1306_WIDTH + 1); // allocate memory for the buf
...
void ssd1306_DrawHLine(unsigned char x, unsigned char y, unsigned char sx)
{
  ...
  ssd1306_buf1 = malloc(sx + 1);       // allocate memory for the buf
  ssd1306_buf1[0] = SSD1306_DATA_MODE; // fist item "send data mode"  
